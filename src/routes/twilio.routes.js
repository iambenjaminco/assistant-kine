    ...
    try {
        ...
        try {
            ...
            if (session.step === "BOOK_PICK_ALT") {
                ...
                const bookingAltActionKey = `BOOK_ALT:${slot.calendarId}:${new Date(slot.start).toISOString()}:${session.phone}`;

                if (hasProcessedAction(session, bookingAltActionKey)) {
                    logWarn("BOOK_ALT_ALREADY_PROCESSED", {
                        callSid,
                        bookingAltActionKey,
                    });
                ...
                markProcessedAction(session, bookingAltActionKey);
