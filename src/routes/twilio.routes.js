...
function sayGoodbye(vr) {
    vr.say({ language: "fr-FR" }, PHRASES.goodbye || "À bientôt. Au revoir.");
    vr.hangup();
}
...
function getCabinetOrFail(vr) {
    ...
    if (!cabinet) {
        vr.say({ language: "fr-FR" }, "Configuration cabinet invalide.");
        vr.hangup();
        return null;
    }
...
router.post("/voice", async (req, res) => {
    ...
    if (
        ...
        g.say({ language: "fr-FR" }, "Très bien, retour au menu principal.");
        g.say({ language: "fr-FR" }, session.lastPrompt);
        return res.type("text/xml").send(vr.toString());
    }
    ...
    if (!hasInput && session.lastPrompt) {
        if (session.skipSilenceOnce) {
            ...
            if (session.noInputCount === 1) {
                const g = gatherSpeech(vr, "/twilio/voice");
                g.say({ language: "fr-FR" }, "Vous êtes toujours là ?");
                g.say({ language: "fr-FR" }, session.lastPrompt);
                return res.type("text/xml").send(vr.toString());
            }
    ...
    try {
        ...
        if (session.step === "ACTION") {
            ...
            if (wantsModify) {
                ...
                const g = gatherSpeech(vr, "/twilio/voice");
                g.say({ language: "fr-FR" }, "Très bien.");
                g.say({ language: "fr-FR" }, session.lastPrompt);
                return res.type("text/xml").send(vr.toString());
            }
            ...
            if (text.includes("rendez") || text.includes("rdv")) {
                ...
                const g = gatherSpeech(vr, "/twilio/voice");
                g.say({ language: "fr-FR" }, "Je n’ai pas compris votre demande.");
                g.say({ language: "fr-FR" }, session.lastPrompt);
                return res.type("text/xml").send(vr.toString());
            }
            ...
            const g = gatherSpeech(vr, "/twilio/voice");
            g.say({ language: "fr-FR" }, "Désolé, je n’ai pas compris.");
            g.say({ language: "fr-FR" }, session.lastPrompt);
            return res.type("text/xml").send(vr.toString());
        }
        ...
        if (session.step === "ACTION_LISTEN") {
            ...
            vr.say({ language: "fr-FR" }, session.lastPrompt);
        ...
        if (session.step === "BOOK_PICK_SLOT") {
            ...
            if (wantsRepeat) {
                ...
                vr.say({ language: "fr-FR" }, "Je répète.");
                vr.say(
                    { language: "fr-FR" },
            ...
            if (choice === null) {
                ...
                if (!a) {
                    session.step = "BOOK_WELCOME";
                    setPrompt(session, "");
                    vr.say({ language: "fr-FR" }, "On recommence.");
                    vr.redirect({ method: "POST" }, "/twilio/voice");
                    return res.type("text/xml").send(vr.toString());
                }

                vr.say({ language: "fr-FR" }, "Je n’ai pas compris.");
                vr.say(
                    { language: "fr-FR" },
                    `Dites "premier" pour ${formatSlotFR(a.start)}${a.practitionerName ? ` avec ${a.practitionerName}` : ""}, ou "deuxième" pour ${formatSlotFR(b.start)}${b.practitionerName ? ` avec ${b.practitionerName}` : ""}.`
                );
        ...
        if (session.step === "BOOK_ASK_NAME") {
            ...
            if (!name) {
                setPrompt(session, "Quel est votre nom et prénom ?");
                const g = gatherSpeech(vr, "/twilio/voice");
                g.say({ language: "fr-FR" }, "Je n’ai pas compris.");
                g.say({ language: "fr-FR" }, session.lastPrompt);
                return res.type("text/xml").send(vr.toString());
            }
        ...
        if (session.step === "BOOK_ASK_PHONE") {
            ...
            if (result.ok) {
                vr.say({ language: "fr-FR" }, PHRASES.confirmed || "C’est confirmé.");
                vr.say(
                    { language: "fr-FR" },
            ...
            vr.say({ language: "fr-FR" }, statusMsg);
            vr.say(
                { language: "fr-FR" },
        ...
        if (session.step === "BOOK_PICK_ALT") {
            ...
            if (!slot || !slot.calendarId) {
                vr.say(
                    { language: "fr-FR" },
                    "Ce créneau n’est plus disponible pour le moment."
                );
                sayGoodbye(vr);
                clearSession(callSid);
                return res.type("text/xml").send(vr.toString());
        ...
        if (session.step === "MODIFY_FIND_APPT") {
            ...
            if (!found) {
                vr.say({ language: "fr-FR" }, "Je ne retrouve pas votre rendez-vous avec ce numéro. Merci de rappeler votre numéro pour vérification.");
                session.phone = "";
                session.foundEvent = null;
                session.step = "MODIFY_ASK_PHONE";
                setPrompt(session, "Quel est votre numéro de téléphone ?");
            ...
            g.say(
                { language: "fr-FR" },
                `J’ai trouvé un rendez-vous le ${formatSlotFR(found.startISO)}.`
            );
            g.say({ language: "fr-FR" }, session.lastPrompt);
            return res.type("text/xml").send(vr.toString());
        }
        ...
        if (session.step === "MODIFY_CONFIRM_FOUND") {
            ...
            if (no) {
                ...
                setPrompt(session, "Quel est votre numéro de téléphone ?");
                const g = gatherSpeech(vr, "/twilio/voice");
                g.say({ language: "fr-FR" }, "Très bien, redonnez-moi votre numéro pour vérification.");
                g.say({ language: "fr-FR" }, session.lastPrompt);
                return res.type("text/xml").send(vr.toString());
            }
            ...
            if (!found) {
                vr.say({ language: "fr-FR" }, "Je ne retrouve plus votre rendez-vous.");
                sayGoodbye(vr);
                clearSession(callSid);
                return res.type("text/xml").send(vr.toString());
            }
        ...
        if (session.step === "MODIFY_PROPOSE_NEW") {
            ...
            vr.say({ language: "fr-FR" }, "D’accord. Je vous propose deux nouveaux créneaux.");
            const cleaned = cleanProposeSpeech(proposeSpeech);
            if (cleaned) vr.say({ language: "fr-FR" }, cleaned);
        ...
        if (session.step === "MODIFY_PICK_NEW") {
            ...
            if (result.ok) {
                vr.say({ language: "fr-FR" }, "C’est modifié et confirmé.");
                vr.say(
                    { language: "fr-FR" },
                    `${formatSlotFR(slot.start)}${slot.practitionerName ? ` avec ${slot.practitionerName}` : ""}.`
                );
        ...
        if (session.step === "CANCEL_CONFIRM_FOUND") {
            ...
            const g = gatherSpeech(vr, "/twilio/voice");
            g.say({ language: "fr-FR" }, "Votre rendez-vous est annulé.");
            g.say({ language: "fr-FR" }, session.lastPrompt);
            return res.type("text/xml").send(vr.toString());
        }
        ...
        if (session.step === "CANCEL_ASK_REBOOK") {
            ...
            if (no) {
                vr.say({ language: "fr-FR" }, "Très bien.");
                sayGoodbye(vr);
                clearSession(callSid);
                return res.type("text/xml").send(vr.toString());
            }
        ...
        // Fallback
        // =========================
        return handleRetry(vr, res, session, callSid);
    } catch (err) {
